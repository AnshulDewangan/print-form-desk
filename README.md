...
- Supabase Auth support is prepared for public Google login and email magic links. The client keeps its session and sends its access token to the Worker; the Worker asks Supabase to validate the token and stores only a namespaced provider ID in D1. Supabase settings are intentionally absent until the owner creates the project and configures callback URLs and Google OAuth. ChatGPT sign-in remains available for private Site review.
