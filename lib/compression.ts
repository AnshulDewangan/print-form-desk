...
export async function compressToLimit<T extends { size: number }>(
  ...
  let low = 0.05,
    high = 0.95;
  const full = await encode(high);
  if (full.size <= limit) return full;
  let best = await encode(low);
  if (best.size > limit)
    throw new Error(
      'Cannot meet the file-size limit at these dimensions. Increase the limit or reduce the pixel dimensions.',
    );
  for (let i = 0; i < 9; i++) {
    const mid = (low + high) / 2,
      encoded = await encode(mid);
