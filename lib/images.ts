...
export function renderCanvas(
  image: HTMLImageElement,
  s: Settings,
  preview: number | false = false,
): HTMLCanvasElement {
  validateSettings(s);
  const canvas = document.createElement('canvas');
  const scale = preview
    ? Math.min(1, preview / Math.max(s.width, s.height))
    : 1;
  ...
  const turn = s.rotation % 180 !== 0,
    w = turn ? image.naturalHeight : image.naturalWidth,
    h = turn ? image.naturalWidth : image.naturalHeight;
...
export async function encodeJPEG(asset: Asset): Promise<Blob> {
  ...
  try {
    return await compressToLimit(
      (quality) => jpeg(canvas, quality),
      asset.settings.maxKB * 1000,
    );
  } catch (e) {
    throw new Error(`${asset.name}: ${(e as Error).message}`);
  }
}
export async function encodeForPrint(
  asset: Asset,
  original = false,
): Promise<Blob> {
  if (!original) return jpeg(renderCanvas(asset.image, asset.settings), 0.95);
  ...
  return jpeg(canvas, 0.95);
}
export function filename(name: string, index: number) {
